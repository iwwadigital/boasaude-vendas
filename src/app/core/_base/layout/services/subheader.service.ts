...
export class SubheaderService {
	...
	getBreadcrumbs(menus: any) {
		...
		// Alteracoes minha - Feito para pegar tanto as views quanto as paginas normais
		url = `/${url}`; 
		let split_url:any = url.split('/');
		if(!isNaN(split_url[split_url.length - 1])){ // Verifico se o ultimo indice e um inteiro
